...
2. datagrip    ， jetbrains出品，优点一个软件连接对个数据库；操作习惯跟pycharm类似；pycharm pro集成。缺点用户少；驱动不好下。
"""
...
database工具使用
展开目录，找到  表。
schemas 模型，理解为打的仓库，默认有一个仓库main， main仓库下是我们建的表。
sqlite-masters是数据库系统内置表，不用关注它，不能删除。
不用关注collations文件夹
双击表查看表数据。
图形化界面 加减号增加修改数据，注意修改完需要submit提交 。
点击connect图标，打开sql命令行工具，可以在里面写sql语句，点击execute按钮执行，得到结果集。
