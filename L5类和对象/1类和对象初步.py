...
print('圆的面积', r2*r2)
print('圆的面积', r3*r3)
# 2. 函数 分装功能，抽象出解决问题的公共过程。调用时传入真实数据
...
# 2. 函数  学生管理作业，先获取用户输入（假设用户要修改学生）先展示学生列表，再接收用户输入新学生的信息，再代码修改数据，最后提示修改成功。
...
# 调用对象方法
print(stu1.name)
stu1.print_score()      # 本质 Student.print_score(stu1)
stu2.print_sex()
...
封装(多态；继承)：类的三大特性之一。 类的封装比函数的封装更加抽象高级，因为类不光包括功能，还包括数据结构。
