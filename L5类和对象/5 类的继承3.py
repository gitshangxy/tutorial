# 继承 > 重写   super()  超级的    (了解)
# 引题1：比如一个计算机系有 会计 电商方向 软件方向， 它们的课程70%一样（数学英语C语言），其它30%不一样（电商seo优化、软件方向xx编程框架），子类跟父类方法部分相同。而上节课的例子run方法是子类完全重写覆盖父类方法。
...
super():调用父类。重写父类属性、方法，减少了代码量。专为继承设计。子类调用完父类相同之处后，个性化的东西可以另外写。
语法：院士写法super(子类名，子类实例)，省略参数super() py2不支持。
优点：方便调用父类中的东西；提高代码复用；父类名变化时子类不用过多修改，好维护。
