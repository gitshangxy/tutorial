# 综合项目：京东评论情感化分析（老师写的版本后添加 未完善）
import pymysql
import jieba
import pygal
from wordcloud import WordCloud
from L46数据分析和图表可视化.dict.degree_dict import degree_dict
...
def my_evaluate(positive_num=0, pos_degree_num=1, negertive_num=0, neg_degree_num=1):
    """
    我的评价
    自定计算公式，自定判断区间，返回用户评论趋势，商品销售预测结果
    :return:
    """
    score = positive_num*pos_degree_num - neg_degree_num*neg_degree_num
    if 0< score <10:
        result = '一般'
    elif score > 50:
        result = '喜欢'
...
"""
github中搜索关键字可以获取到相关字典。
词性：
1. 过滤掉 "停用词"、"停止词"
2. 搜索"情感词汇" "积极词汇字典" "消极词汇"
3. 读取字典到变量。
4. for循环分词后的结果，跟字典变量内容做比对。
"""
