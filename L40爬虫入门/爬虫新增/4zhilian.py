...
# 2. 如果一个网站是有分页的，这种网站可能是静态网站(网页源代码中就有数据)，也有可能是动态网站(网页源代码中没有数据，是一堆的<script>标签)，如果是静态网站直接使用re，xpath等解析网页源代码即可；如果是动态网站，先分析是不是有json接口；如果没有json接口，再去源代码中查看一下json数据是否包含在<script>标签中；如果以上都无法获取数据，再使用selenium动态加载js代码，获取完整的网页源代码并解析；

class ZLspoder(object):
    def __init__(self):
    ...
    def get_json_str(self):
        ...
        response = requests.get(url=url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.109 Safari/537.36'})
    ...
    def parse_json(self, json_str):
        ...
        if results:
            for zhiwei_dict in results:
                ...
                print(welfare, salary, jobName, detail_url, content)
            # 继续请求后续的数据
            self.offset += 90
            j = self.get_json_str()
            self.parse_json(j)
    ...
    def get_detail(self, detail_url):
        ...
        :return:
        """
        html = requests.get(detail_url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.109 Safari/537.36'}).text
