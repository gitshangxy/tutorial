...
"""
运行程序。结果卡死。两人都拿到了一个资源，但第二个资源都在对方手里，但由于自己的功能未完成，导致自己拿到的资源没释放。两个线程并不聪明，不会把自己的资源先借给对方，而是两方一直等待对方。

避免这种情况发生，改为递归锁。 threading.Lock()改为threading.RLock()
"""

"""
既然用户可以在代码中自由加锁控制，确保线程安全，那么为什么还要GIL？
因为GIL处于解释器层面，轮询查看有没有待销毁的垃圾，相当于执行某一项功能的线程。如果垃圾回收功能的线性跟代码
"""