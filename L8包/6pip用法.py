...
"""
pip: python install packge, python三方包管理工具。安装python解释器时已自带。目录已添加到环境变量中。
包管理工具： 包时别人写好的代码。经常有这种情况，比如负责爬虫框架功能的A包，里面引用了负责解析网页的B包，B包引用了更加基础底层的C包。包关系成树装引用。B包依赖C包。直接使用A包，运行报错缺少B包，缺少依赖包
...
"""
pip list   # 输出安装过的三方包的列表。pip 工具本身就是一个包
 pip search  关键字   # 搜索包含关键字的包名。
 pip install  包名   # （常用）安装包。安装包的本质是从pypi下载，解压复制粘贴到C:\python36\Lib\site-package  下。 默认安装包的最新版本。
 pip install requests==2.19.0

 pip  uninstall 包名    # 卸载包      # 安装指定版本的包
...
永久修改（推荐）：C:/Users/用户名下，新建pip文件夹，pip文件夹下新建pip.ini
写入下面两行
[global]
...
激活虚拟环境后可以pip install跟目录配合的包， python app.py。如果有多个项目，就生成多个虚拟环境一一搭配。好处，实现了项目隔离。
"""
...
4. cache  缓存。 之前已经下载过安装包，再次
"""



"""
在pycharm中进行如上操作(详情见下一小节)
"""
...
优点：适合任何操作系统和python版本。缺点:有些包源代码
"""